...
/**
 * Save a single trip to Firestore (new format, one document per trip)
 */
export const saveTripToCloud = async (uid: string, trip: Trip | TripData): Promise<void> => {
  ...
  await setDoc(
    ref,
    {
      ...
      updatedAt: serverTimestamp(),
      schemaVersion: 2, // Version 2 = multi-trip support
    },
    { merge: true }
  );
  ...
  try {
    ...
    // If subcollection writes are blocked by rules, keep backup working via trip-doc payload.
    await setDoc(
      ref,
      {
        data: sanitizeForFirestore(data),
    ...
    console.warn('saveTripToCloud fallback to trip document payload', error);
  }
};

/**
 * Load all trips for a user (new format)
 */
    ...
    snap.docs.map(async (tripDoc) => {
      ...
      if (!tripData) {
        try {
          ...
          console.warn('loadAllTripsFromCloud fallback to setup-only trip', error);
        }
      }
...
export const loadTripFromCloud = async (uid: string, tripId?: string): Promise<TripData | null> => {
  ...
  try {
    ...
    console.warn('loadTripFromCloud fallback to setup-only trip', error);
    return {
      setup: (payload?.setup as TripData['setup']) || null,
      deletedExpenseMap: {},
      expenses: [],
...
export const deleteTripFromCloud = async (uid: string, tripId: string): Promise<void> => {
  ...
  try {
    ...
    // Fallback when subcollection access is blocked: at least remove parent trip doc.
    await deleteDoc(ref);
    console.warn('deleteTripFromCloud fallback to parent-doc delete', error);
  }
};